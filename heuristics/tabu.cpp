...
TabuList::TabuList(int n, size_t max_size) {
    n_ = n;
    max_size_ = max_size;
    is_tabu_.resize(n *n , false);
    tabu_list_.resize(max_size);
}

bool TabuList::contains(const std::pair<int, int> &move) {
...
void TabuList::insert(const std::pair<int, int> &move) {
    size_t idx = index(move);
    if (is_tabu_[idx]) return;
    //Add move to tabu list
    tabu_list_.push_back(move);
    is_tabu_[idx] = true;
    //Check if tabu list is too large, if so, get rid of oldest
    if (tabu_list_.size() > max_size_) {
        size_t old_idx = index(tabu_list_.front());
        is_tabu_[old_idx] = false;
        tabu_list_.pop_front();
    }
}
void TabuList::reset() {
    std::fill(is_tabu_.begin(), is_tabu_.end(), false);
    tabu_list_.clear();
}

size_t TabuList::index(const std::pair<int, int> &move) const {
    return static_cast<size_t>(move.first) * n_ + move.second;
}
...
void Tabu::perform_shift(ALBPSolution &sol, int task, int task_idx, int old_station,int new_station) {
    ...
    sol.overloads_[new_station] =  std::max(sol.loads[new_station],0);
    sol.overloads_[old_station] =  std::max(sol.loads[old_station],0);
        ...
        for (int i=0; i < sol.station_assignments[s].size(); i++) {
            ...
            for (int j = sol.earliest[task]; j < sol.latest[task]; j++ ) {
                if( j != (s)){
                    ...
                    if (obj < target_c) {
                        target_c = obj;
                        local_best = TabuMove(current_sol, obj, std::pair(task,s), std::pair(task,j), i);
                    }
                }
            }
    ...
    for (int i=0; i < current.station_assignments[s].size(); i++) {
        ...
        for (int j = current.earliest[task]; j < current.latest[task]; j++ ) {
            if( j != (s)){
                ...
                if ( !tabu_.contains(move)  && obj <=  local_obj) {
                    local_obj = obj;
    ...
    for (int task_idx=0; task_idx < current.station_assignments[s].size(); task_idx++) {
        ...
        for (int new_station = current.earliest[task]; new_station < current.latest[task]; new_station++ ) {
            for (int new_task_idx=0; new_task_idx < current.station_assignments[new_station].size(); new_task_idx++){
                ...
                if( new_station != (s) && current.earliest[new_task]<= s && current.latest[new_task]>= s) {
                    ...
                    if ( !tabu_.contains(move1)  && !tabu_.contains(move2) && obj <=  local_obj) {
                        local_obj = obj;
...
void Tabu::do_move(ALBPSolution & sol, const TabuMove &move, const bool update_tabu= true) {
    if (move.move != std::pair<int,int>(-1,-1)) {
        sol = move.sol;
        if (update_tabu) tabu_.insert(move.reversal);
